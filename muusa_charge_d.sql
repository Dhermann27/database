DROP TRIGGER IF EXISTS muusa_charge_d&
CREATE TRIGGER muusa_charge_d AFTER DELETE ON muusa_charge
FOR EACH ROW
BEGIN
	...
	DECLARE cur1 CURSOR FOR
		SELECT SUM(s1.amount), s1.timestamp FROM
			...
			WHERE h.camperid=c.id AND h.amount<0 AND c.familyid=familyid AND h.year=OLD.year
				UNION ALL
			SELECT -(bsp.registration_amount+bsp.housing_amount), bsp.created_at 
			FROM muusa_byyear_staff bsp
			WHERE bsp.familyid=familyid AND bsp.year=OLD.year) s1
	...
	SELECT c.familyid, GROUP_CONCAT(ya.id) INTO familyid, yearattendingids 
		...
		WHERE c.id=OLD.camperid AND c.familyid=cp.familyid AND ya.camperid=cp.id AND ya.year=OLD.year 
		GROUP BY c.familyid;

	IF familyid != 0 THEN
		SELECT muusa_getnowbalance(familyid, OLD.year) INTO charges;
